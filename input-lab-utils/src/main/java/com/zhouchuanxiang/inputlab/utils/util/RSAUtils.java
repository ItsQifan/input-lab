...
/** *//**
    ...
    public static String encryptByPublicKey(String data, String publicKey)
        ...
        //byte[] dateBytes = Base64Utils.decode(data);

        byte[] dateBytes = data.getBytes();

        int inputLen = dateBytes.length;
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        int offSet = 0;
        byte[] cache;
        int i = 0;
        ...
        while (inputLen - offSet > 0) {
            if (inputLen - offSet > MAX_ENCRYPT_BLOCK) {
                cache = cipher.doFinal(dateBytes, offSet, MAX_ENCRYPT_BLOCK);
            } else {
                cache = cipher.doFinal(dateBytes, offSet, inputLen - offSet);
            }
            out.write(cache, 0, cache.length);
            i++;
